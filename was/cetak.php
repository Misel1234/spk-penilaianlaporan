...
$pdf->SetLineWidth(0);
$pdf->ln(1);
$pdf->Cell(25,0.9,"SEKSI PENGAWASAN",0,0,'C');
$pdf->ln(1);
...
$query = mysqli_query($koneksi, "SELECT * FROM data_kehadiran WHERE seksi = 'Seksi Pengawas'");
...
$pdf->ln(1);
$pdf->Cell(25,0.7,"LAPORAN EVALUSI KINERJA",0,0,'C');
$pdf->ln(1);
$pdf->SetFont('Arial','B',10);
$pdf->Cell(1.5, 0.8, 'No', 1, 0, 'C');
$pdf->Cell(8, 0.8, 'Nama Bidang', 1, 0, 'C');
$pdf->Cell(4, 0.8, 'Evaluasi Kinerja', 1, 0, 'C');
$pdf->Cell(4, 0.8, 'Keterangan', 1, 0, 'C');
$pdf->Cell(8, 0.8, 'Hasil SPK', 1, 0, 'C');
$pdf->ln();

$no = 1;
//JUMLAH DATA TRAINING/DATASET
$query4 = mysqli_query($koneksi, "SELECT count(*) AS jumlah FROM data_training_pengawasan");
$jumlah = mysqli_fetch_array($query4);

//MENGHITUNG JUMLAH DATA TIDAK BAIK, KURANG BAIK DAN BAIK
$T      = mysqli_query($koneksi, "SELECT count(*) AS tidak FROM data_training_pengawasan WHERE keterangan = 'Tidak Baik'");
$hasilT = mysqli_fetch_array($T);
$nilaiT = $hasilT['tidak'] / $jumlah['jumlah'];

$B      = mysqli_query($koneksi, "SELECT count(*) AS baik FROM data_training_pengawasan WHERE keterangan = 'Baik'");
...
$c1a   = mysqli_query($koneksi, "SELECT COUNT(*) AS c1a FROM data_training_pengawasan WHERE c1 = '$data1[evaluasi_kinerja]' AND keterangan = 'Tidak Baik'");
$h_c1a = mysqli_fetch_array($c1a);

$c1b   = mysqli_query($koneksi, "SELECT COUNT(*) AS c1b FROM data_training_pengawasan WHERE c1 = '$data1[evaluasi_kinerja]' AND keterangan = 'Baik'");
$h_c1b = mysqli_fetch_array($c1b);

$hasil_tidak = $h_c1a['c1a'] / $hasilT['tidak'] * $nilaiT;
$hasil_baik  = $h_c1b['c1b'] / $hasilB['baik'] * $nilaiB;
...
if($hasil_tidak > $hasil_baik){
  ...
  $pdf->Cell(1.5, 0.8, $no , 1, 0, 'C');
  $pdf->Cell(8, 0.8, $data1['nama_bidang'],1, 0, 'C');
  $pdf->Cell(4, 0.8, $data1['evaluasi_kinerja'], 1, 0,'C');
  $pdf->Cell(4, 0.8, $data1['keterangan'], 1, 0,'C');
  $pdf->Cell(8, 0.8, $keterangan, 1, 0,'C');
  $pdf->ln();
  $no++;
}
