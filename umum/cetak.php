...
$pdf->SetLineWidth(0);
$pdf->ln(1);
$pdf->Cell(25,0.9,"SEKSI UMUM",0,0,'C');
$pdf->ln(1);
...
$query = mysqli_query($koneksi, "SELECT * FROM data_kehadiran WHERE seksi = 'Seksi Umum'");
...
$pdf->ln(1);
$pdf->Cell(25,0.7,"LAPORAN PENGADAAN DAN INVENTARIS BARANG",0,0,'C');
$pdf->ln(1);
$pdf->SetFont('Arial','B',10);
$pdf->Cell(1.5, 0.8, 'No', 1, 0, 'C');
...
$no = 1;
                  ...
                  $query4 = mysqli_query($koneksi, "SELECT count(*) AS jumlah FROM data_training_umum");
                  $jumlah = mysqli_fetch_array($query4);

                  //MENGHITUNG JUMLAH DATA TIDAK BAIK, KURANG BAIK DAN BAIK
                  $T       = mysqli_query($koneksi, "SELECT count(*) AS perbaikan FROM data_training_umum WHERE keterangan = 'Tidak Aktif'");
                  $hasilT  = mysqli_fetch_array($T);
                  $nilaiT  = $hasilT['perbaikan'] / $jumlah['jumlah'];

                  $B       = mysqli_query($koneksi, "SELECT count(*) AS tidak_ada FROM data_training_umum WHERE keterangan = 'Aktif'");
                  ...
                  while($data1 = mysqli_fetch_array($query)){
                    ...
                    $c1a   = mysqli_query($koneksi, "SELECT COUNT(*) AS c1a FROM data_training_umum WHERE c1 = '$data1[kondisi]' AND keterangan = 'Tidak Aktif'");
                    $h_c1a = mysqli_fetch_array($c1a);

                    $c1b   = mysqli_query($koneksi, "SELECT COUNT(*) AS c1b FROM data_training_umum WHERE c1 = '$data1[kondisi]' AND keterangan = 'Aktif'");
  ...
  $pdf->Cell(1.5, 0.8, $no , 1, 0, 'C');
  $pdf->Cell(4, 0.8, $data1['nama_barang'],1, 0, 'C');
  $pdf->Cell(4, 0.8, $data1['jumlah_barang'],1, 0, 'C');
  $pdf->Cell(4, 0.8, $data1['kondisi'], 1, 0,'C');
